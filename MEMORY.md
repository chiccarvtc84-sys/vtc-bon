...
| `voice-extract` | oui | **7** | ACTIVE — Google Gemini 2.5 Flash (free tier), dictée vocale intelligente — migré depuis Claude Sonnet 4.6 le 2026-05-01. Notes : (a) `gemini-2.0-flash` était déprécié en 2026 (404 NOT_FOUND) → switch vers `gemini-2.5-flash` (v6). (b) `gemini-2.5-flash` a un mode "thinking" activé par défaut qui consommait le budget tokens et tronquait le JSON → désactivé via `thinkingConfig: { thinkingBudget: 0 }` (v7). |
